...
//    }
    ...
    @Test
    public void mLetterScope() throws ParseException {
        Scope s = new Scope();
        try {
        ...
        String string1 = "platform = 2 && (appVersion > '9.5.0') && !(sdkVersion > '1.0')";

        String s2 = "1 < 2 && 2 > 1 && 2 >= 2";

        String s3 = "1 > 2 || 2 < 1 && 2 < 2 && '2.3'< '9.5.0'";

        Variable cd = s.create("appVersion");
        cd.setValue(12.f);
        cd.setStringValue("9.5.0");

        String s4 = "appVersion < '9.5.0'";
        Expression parse = Parser.parse(s4, s);

        System.out.println(VersionUtil.compare("9.4.0", "9.5.0"));

        System.out.println("appversion:" +    parse.evaluate());


        long b = System.currentTimeMillis();
        for(int i =0 ;i< 1000000;i++) {
//            Expression parse1 = Parser.parse(s2, s);
          ///  System.out.println();
         //   VersionUtil.compare("9.5.0","9.5.0");
//                    parse1.evaluate();

//            Expression parse = Parser.parse("'2.3'< '9.5.0'", s);

     //       Expression parse = Parser.parse("'2.3'< '9.5.0'", s);


//            String s5 = "1 < 2 && 2 < 1 && 2 < 2 && appVersion < '9.5.0'";
//            Expression parse2 = Parser.parse(s5, s);
//
//            System.out.println("++++"+    parse2.evaluate());
            //   Expression parse = Parser.parse(s3, s);
    ...
    //    System.out.println("++++"+    parse.evaluate());
        ...
        System.out.println("===" + 100000f/(a - b));
  ...
  @Test
  public void fullTestForVersionString() throws ParseException {

      Scope s = new Scope();

      Variable appId = s.create("appId");

      Variable appVersion = s.create("appVersion");

      Variable platform = s.create("platform");

      String case1 = "platform == 2 && (appVersion > '9.5.0') && (appId >= 1)";

      appId.setValue(1);
      appVersion.setStringValue("9.6.0");
      platform.setValue(1);

      assertFalse((Double) Parser.parse(case1, s).evaluate() > 0);

      appId.setValue(1);
      appVersion.setStringValue("9.6.0");
      platform.setValue(2);

      assertTrue((Double) Parser.parse(case1, s).evaluate() > 0);

      case1 = "platform == 2 && (appVersion > '9.5.0') && (appId >= 1)";
      appId.setValue(1);
      appVersion.setStringValue("9.6.1");
      platform.setValue(2);
      ...
      case1 = "platform == 2 || (appVersion > '9.5.0') && (appId >= 1)";
      appId.setValue(0);
      appVersion.setStringValue("9.4.0");
      platform.setValue(2);
      ...
      case1 = "platform != 2 || ((appVersion > '9.5.0')  && appVersion <= '9.5.0' && (appId >= 1))";
      appId.setValue(1);
      appVersion.setStringValue("9.5.0");
      platform.setValue(2);

      assertFalse((Double) Parser.parse(case1, s).evaluate() > 0);


      case1 = "platform == 2 || ((appVersion > '9.5.0') && (appId >= 1)) && appVersion <= '9.5.1'";
      appId.setValue(1);
      appVersion.setStringValue("9.5.0");
      platform.setValue(2);
